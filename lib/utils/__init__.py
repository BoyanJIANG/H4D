...
def get_optimizer(model, optim_type, lr, weight_decay, momentum):
    optim_list = [{"params": [p for p in model.parameters() if p.requires_grad]}]
    if optim_type in ['sgd', 'SGD']:
        opt = torch.optim.SGD(lr=lr, params=optim_list, momentum=momentum)
    elif optim_type in ['Adam', 'adam', 'ADAM']:
        opt = torch.optim.Adam(lr=lr, params=optim_list, weight_decay=weight_decay)
    else:
