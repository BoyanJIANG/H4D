...
def compute_error_accel(joints_gt, joints_pred, vis=None, add_mean=True):
    """
...
def compute_error_verts(pred_verts, target_verts=None, target_theta=None, add_mean=True):
    """
    ...
    if target_verts is None:
        SMPL_MODEL_DIR = 'data/SMPL_NEUTRAL.pkl'
        ...
        betas = torch.from_numpy(target_theta[:,72:]).to(device)
        pose = torch.from_numpy(target_theta[:,:72]).to(device)
